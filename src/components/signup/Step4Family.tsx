...
const Step4Family = ({ data, onNext, onBack }: StepProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate Parents Contact Number if provided
    if (formData.parentsContactNo) {
      const phoneError = validatePhoneNumber(formData.parentsContactNo, "Parents Contact Number");
      if (phoneError) {
        setValidationErrors(new Set(["parentsContactNo"]));
        toast({
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <Input
            ...
            onChange={(e) => {
              setFormData({ ...formData, parentsContactNo: e.target.value });
              // Clear error when user starts typing
              if (validationErrors.has("parentsContactNo")) {
                setValidationErrors(new Set(Array.from(validationErrors).filter(f => f !== "parentsContactNo")));
              }
            }}
            className={cn(
