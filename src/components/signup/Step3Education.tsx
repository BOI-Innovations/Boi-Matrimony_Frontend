...
const Step3Education = ({ data, onNext, onBack }: StepProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate College/Institution
    const collegeError = validateNameField(formData.collegeInstitution, "College / Institution");
    if (collegeError) {
      setValidationErrors(new Set(["collegeInstitution"]));
      toast({
    ...
    const organizationError = validateNameField(formData.organizationName, "Organization Name");
    if (organizationError) {
      setValidationErrors(new Set(["organizationName"]));
      toast({
        title: "Validation Error",
    ...
    // Validate Work City
    const workCityError = validateNameField(formData.workCity, "Work City");
    if (workCityError) {
      setValidationErrors(new Set(["workCity"]));
      toast({
        title: "Validation Error",
        description: workCityError,
        variant: "destructive",
      });
      return;
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                setFormData({ ...formData, collegeInstitution: e.target.value });
                // Clear error when user starts typing
                if (validationErrors.has("collegeInstitution")) {
                  setValidationErrors(new Set(Array.from(validationErrors).filter(f => f !== "collegeInstitution")));
                }
              }}
              className={cn(
                validationErrors.has("collegeInstitution") && "border-red-500 border-2 focus:border-red-500"
              )}
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                ...
                // Clear error when user starts typing
                if (validationErrors.has("organizationName")) {
                  setValidationErrors(new Set(Array.from(validationErrors).filter(f => f !== "organizationName")));
                }
              }}
              className={cn(
                validationErrors.has("organizationName") && "border-red-500 border-2 focus:border-red-500"
              )}
          ...
          <div className="space-y-2">
            ...
            {selectedState?.districts && selectedState.districts.length > 0 ? (
              ...
              <Input
                ...
                value={formData.workCity}
                onChange={(e) => {
                  setFormData({ ...formData, workCity: e.target.value });
                  // Clear error when user starts typing
                  if (validationErrors.has("workCity")) {
                    setValidationErrors(new Set(Array.from(validationErrors).filter(f => f !== "workCity")));
                  }
                }}
                className={cn(
                  validationErrors.has("workCity") && "border-red-500 border-2 focus:border-red-500"
                )}
