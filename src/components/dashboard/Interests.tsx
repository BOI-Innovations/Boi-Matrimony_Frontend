            ...
            >
                ...
                <>
                    ...
                    <Button 
                        ...
                        onClick={() => onWithdraw(connectionInfo?.requestId)}
                    >
                        Withdraw
                    </Button>
                </>
...
const Matches = ({ onViewProfile }: MatchesProps = {}) => {
  ...
  const fetchConnectionStatus = async () => {
    ...
    try {
      ...
      sent.forEach((req: any) => {
        switch (req.status) {
          case "PENDING":
            statusMap[req.receiverId] = { state: "pending", requestId: req.id };
            break;
          case "ACCEPTED":
  ...
  // ----------------------------------------------------
  const handleWithdraw = async (requestId: number | undefined) => {
    if (!token || requestId === undefined) return;

    try {
      const res = await fetch(
        `${baseUrl.replace("9096", "9099")}/api/connections/withdraw/${requestId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
