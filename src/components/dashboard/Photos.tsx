    ...
    const fetchGalleryImages = async () => {
      ...
      try {
        ...
        if (res.ok && data.statusCode === "200") {
          ...
          const loadImg = async (img: any) => {
            try {
              const imgRes = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/images/getGalleryImageById?id=${img.id}`, { headers: { Authorization: `Bearer ${token}` } });
              if (!imgRes.ok) throw new Error("Failed to load gallery image");
              const blob = await imgRes.blob();
              const imgUrl = URL.createObjectURL(blob);
              setGalleryImages((prev) => prev.map((i) => (i.id === img.id ? { ...i, url: imgUrl, loading: false } : i)));
            } catch (error) {
            ...
            {loadingGallery ? (
              ...
              galleryImages.map((img, i) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-smooth rounded-lg">
