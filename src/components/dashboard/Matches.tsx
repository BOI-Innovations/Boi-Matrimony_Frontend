...
const Matches = ({ onViewProfile }: MatchesProps = {}) => {
  ...
  const fetchMatches = async (pageNo = 0) => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/suggestions/profiles?page=${pageNo}&size=10`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const StyledSelect = ({ 
    ...
    <select
      className="relative z-30 flex h-10 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
      value={value || ""}
      onChange={onChange}
      disabled={disabled}
    >
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <Dialog open={showFilters} onOpenChange={setShowFilters}>
        <DialogContent className="w-[90vw] sm:w-full max-w-3xl max-h-[90vh] flex flex-col p-0 gap-0 rounded-xl overflow-visible">
          ...
          <div className="flex-1 px-6 pb-4 pt-2 overflow-y-auto">
            <Tabs value={activeFilterTab} onValueChange={setActiveFilterTab} className="w-full">
              <TabsList className="relative z-20 grid grid-cols-3 md:grid-cols-6 gap-2 mb-4">
