              ...
              >
                {loadingGallery
                  ? Array.from({ length: user.images?.length || 4 }).map((_, i) => (
                      <div
                        key={i}
                        className="flex-shrink-0 w-32 h-32 rounded-lg bg-gray-200 animate-pulse"
                      ></div>
                    ))
                  : galleryImages.length > 0
                  ? galleryImages.map((img, i) => (
                      <div key={img.id || i} className="flex-shrink-0 relative">
                        {img.loading ? (
                          <div className="w-40 h-40 rounded-lg bg-gray-200 animate-pulse"></div>
                        ) : img.url ? (
                          <img
                            src={img.url}
                            alt={`Gallery ${i + 1}`}
                            className="w-40 h-40 object-cover rounded-lg border-2 border-border cursor-pointer hover:opacity-80 transition-opacity"
                            onClick={() => handleImageClick(img.url)}
                          />
                        ) : (
                          <div className="w-40 h-40 rounded-lg bg-gray-100 flex items-center justify-center text-muted-foreground">
                            Failed to load
                          </div>
                        )}
                      </div>
