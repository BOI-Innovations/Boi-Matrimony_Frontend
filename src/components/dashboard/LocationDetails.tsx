...
const LocationDetails = () => {
  ...
  const validateLivingSinceYear = (value: string | number): string | null => {
    ...
    if (isNaN(year)) {
      return "Living Since Year must be a valid number.";
    }
    const currentYear = new Date().getFullYear();
    if (year < 1900 || year > currentYear) {
      return `Living Since Year must be between 1900 and ${currentYear}.`;
    }
    return null;
  };
  ...
  const handleSave = async () => {
    ...
    if (cityError) {
      setValidationErrors(new Set(["city"]));
      toast({
    ...
    if (postalCodeError) {
      setValidationErrors(new Set(["postalCode"]));
      toast({
        title: "Validation Error",
    ...
    // Validate Living Since Year
    const yearError = validateLivingSinceYear(formData.livingSinceYear);
    if (yearError) {
      setValidationErrors(new Set(["livingSinceYear"]));
      toast({
        title: "Validation Error",
